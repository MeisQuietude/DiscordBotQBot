    ...
    async execute(message, args) {
        if (args.length < 2) {
            return message.reply(
                "Need an action (add or rm) and at least one role: [add/rm] [...roles]"
            );
        }
        ...
        for (const role in enteredRoles) {
            const array = isRoleAllowed(role) ? validRoles : invalidRoles;
            array.push(role);
        }
        ...
        Logger.action(
            `Managing role, ${message.member.user.username} ${message.member.user.id}, role ${args[0]} ${enteredRoles}`
        );
    },
};
